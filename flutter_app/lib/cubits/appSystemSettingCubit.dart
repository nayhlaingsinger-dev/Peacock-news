...
class AppConfigurationCubit extends Cubit<AppConfigurationState> {
  ...
  String? getNativeAdsMode() {
    return (state is AppConfigurationFetchSuccess) ? getAppConfiguration().nativeAdsMode : "0";
  }

  int getNativeAdsInterval() {
    if (state is AppConfigurationFetchSuccess) {
      return int.tryParse(getAppConfiguration().nativeAdsInterval ?? "4") ?? 4;
    }
    return 4;
  }
}
