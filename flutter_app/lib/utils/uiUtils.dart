...
// get app theme
  ...
  static String? convertToAgo(BuildContext context, DateTime input, int from) {
    ...
    } else if (diff.inDays >= 365 && from == 1) {
...
//Home Screen - featured sections
  ...
  static showRewardedAds({required BuildContext context, Function? onRewardEarned}) {
    if (context.read<AppConfigurationCubit>().getInAppAdsMode() == "1") {
      if (context.read<AppConfigurationCubit>().checkAdsType() == "google") {
        showGoogleRewardedAd(context, onRewardEarned: onRewardEarned);
      } else {
        showUnityRewardAds(context.read<AppConfigurationCubit>().rewardId()!, onRewardEarned: onRewardEarned);
      }
    } else {
  ...
  static void shareNews(
    ...
    String shareLink = "$shareNavigationWebUrl/$contentType/$slug?language_id=${context.read<AppLocalizationCubit>().state.id}&share=true";
...
Widget nativeAdsShow({required BuildContext context, required int index}) {
  final adConfig = context.read<AppConfigurationCubit>();
  if (adConfig.getInAppAdsMode() == "1" &&
      adConfig.getNativeAdsMode() == "1" &&
      adConfig.checkAdsType() != null &&
      (adConfig.getIOSAdsType() != "unity" || adConfig.getAdsType() != "unity") &&
      index != 0 &&
      index % adConfig.getNativeAdsInterval() == 0) {
    debugPrint("Native ads showing at index: $index");
    return Padding(
        ...
        child: Container(
            ...
            child: adConfig.checkAdsType() == "google" && (adConfig.nativeId() != "")
                ? UiUtils.bannerAdsShow(context: context) // You can replace this with a real Native Ad widget if needed
                : SizedBox.shrink()));
  } else {
